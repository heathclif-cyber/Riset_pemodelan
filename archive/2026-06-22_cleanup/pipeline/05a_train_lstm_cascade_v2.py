...
logger = setup_logger("05a_train_lstm_cascade_v2")
DEVICE = get_lstm_device()

RUN_NAME = "ic32_lstm_cascade_v2"
LGBM_RUN = "ic32_regime_v2"
MAX_SEQ_PER_COIN = 8_000   # cap per coin agar training tetap cepat
...
def load_data_complement(coins: list[str], feat_cols: list[str],
    ...
    for coin in coins:
        ...
        # Exp A (Cascade): semua bars valid, tanpa gate/flat filter
        # Cap per coin untuk training speed
        all_idx = [i for i in range(LSTM_SEQ_LEN - 1, len(X_c))
                   if not (np.isnan(p0_lgbm[i]) and np.isnan(p2_lgbm[i]))]
        if len(all_idx) > MAX_SEQ_PER_COIN:
            step = len(all_idx) // MAX_SEQ_PER_COIN
            all_idx = all_idx[::step][:MAX_SEQ_PER_COIN]
        n_all = len(all_idx)
        for i in all_idx:
            ...
            meta_rows.append({
                ...
                "p0_lgbm": float(p0_lgbm[i]) if not np.isnan(p0_lgbm[i]) else 0.0,
                "p2_lgbm": float(p2_lgbm[i]) if not np.isnan(p2_lgbm[i]) else 0.0,
            })

        sub = y_c[np.array(all_idx) - (LSTM_SEQ_LEN - 1)] if all_idx else y_c[:0]
        sub = y_c[[i for i in all_idx]]
        total_gate += n_all
        total_flat += n_all
        if n_all > 0:
            logger.info(
                f"  [{coin}] all_bars={n_all:,} (capped) | "
                f"LONG={(y_c[[i for i in all_idx]] == 2).mean()*100:.0f}% "
                f"FLAT={(y_c[[i for i in all_idx]] == 1).mean()*100:.0f}% "
                f"SHORT={(y_c[[i for i in all_idx]] == 0).mean()*100:.0f}%"
            )
