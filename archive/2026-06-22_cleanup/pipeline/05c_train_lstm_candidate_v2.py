...
LSTM Candidate Domain untuk ic32_regime_v2.

Hipotesis: LGBM (snapshot) dan LSTM (historian) punya edge berbeda.
Domain filter: p2_lgbm >= CAND_THR OR p0_lgbm >= CAND_THR (CAND_THR=0.55)
LSTM belajar mengevaluasi trajectory 72 bar pada bars yang sudah
dianggap directional oleh LGBM — coverage hampir 100% LGBM signal bars.

Usage:
  python pipeline/05c_train_lstm_candidate_v2.py --all
"""
...
logger = setup_logger("05c_train_lstm_candidate_v2")
DEVICE = get_lstm_device()

RUN_NAME = "ic32_lstm_candidate_v2"
LGBM_RUN = "ic32_regime_v2"

CAND_THR = 0.55  # LGBM candidate threshold — include if p2>=CAND_THR or p0>=CAND_THR
...
def load_data_complement(coins: list[str], feat_cols: list[str],
    ...
    for coin in coins:
        ...
        # Candidate domain: include bars where LGBM is directionally confident
        # p2_lgbm >= CAND_THR (leaning LONG) OR p0_lgbm >= CAND_THR (leaning SHORT)
        # LSTM learns: "given LGBM thinks this is directional, does the 72-bar
        # trajectory support it?" — coverage ~100% of LGBM signal bars
        n_cand = n_incl = 0
        ...
        if n_incl > 0:
            logger.info(
                f"  [{coin}] cand={n_cand:,} incl={n_incl:,} | "
    ...
    meta_df["ts"] = ts
    order = np.argsort(ts)
    logger.info(f"Candidate pool: cand={total_gate:,} -> incl={total_flat:,} sequences")
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  LSTM IC32 CANDIDATE DOMAIN -- {RUN_NAME}")
