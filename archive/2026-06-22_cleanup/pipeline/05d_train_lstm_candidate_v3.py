...
LSTM Candidate Domain v3 -- fitur lebih kuat, SEQ_LEN=24.

Perubahan dari v2:
- 14 fitur baru (avg IC ~0.055 vs v2 ~0.032)
- Drop: ofi_acceleration, ofi_z_score, log_ret_20, volume_delta, btc_h1_return
- Add:  dist_from_8h_high, ema_7_h1, rsi_h4, swing_momentum,
        stochrsi_k, trend_accel_4h, ultra_high_vol, dist_liq_50x_short
- SEQ_LEN = 24 (dari 72): swing H4 biasanya resolve <1 hari
- PURGE = 24 (match SEQ_LEN)

Usage:
  python pipeline/05d_train_lstm_candidate_v3.py --all
"""
...
logger = setup_logger("05d_train_lstm_candidate_v3")
DEVICE = get_lstm_device()

RUN_NAME  = "ic32_lstm_candidate_v3"
LGBM_RUN  = "ic32_regime_v2"
SEQ_LEN   = 24
PURGE_GAP = 24   # >= SEQ_LEN agar input window tidak overlap ke training period
CAND_THR  = 0.55

LSTM_FEATURES = [
    # Dari v2 — IC kuat, tetap dipakai
    "rsi_6",              # IC 0.0722 — terkuat di v2
    "log_ret_5",          # IC 0.0590
    "absorption_z",       # IC 0.0468
    "cvd_momentum_adv",   # IC 0.0342
    "log_ret_1",          # IC 0.0322
    "vol_ratio_20",       # IC 0.0309
    # Baru — IC lebih tinggi dari semua fitur v2 lama yang di-drop
    "dist_from_8h_high",  # IC 0.0773 (lebih kuat dari rsi_6!)
    "ema_7_h1",           # IC 0.0689
    "rsi_h4",             # IC 0.0576
    "swing_momentum",     # IC 0.0557 — named for swing
    "stochrsi_k",         # IC 0.0546
    "trend_accel_4h",     # IC 0.0517
    "ultra_high_vol",     # IC 0.2012 — context signal (extreme vol event)
    "dist_liq_50x_short", # IC 0.0491 — liquidity proximity
]

# Fitur CVD-derived yang perlu per-coin rolling z-score (skala berbeda tiap koin)
...
def focal_alpha(y_train):
    ...
    boosts = {
        0: FOCAL_CFG["alpha_bear_boost"],
        1: FOCAL_CFG["alpha_neu_scale"],
        2: FOCAL_CFG["alpha_bull_boost"],
    }
    return torch.tensor(
...
def load_data(coins: list[str], lgbm_oof: pd.DataFrame):
    ...
    for coin in coins:
        ...
        # Candidate domain: bars di mana LGBM already confident
        n_incl = 0
        for i in range(SEQ_LEN - 1, len(X_c)):
            if np.isnan(p0_lgbm[i]) or np.isnan(p2_lgbm[i]):
                continue
            if p2_lgbm[i] < CAND_THR and p0_lgbm[i] < CAND_THR:
                continue
    ...
    if not X_seqs:
        raise ValueError("No sequences built. Check LGBM OOF + features_v3.")
...
def train_one_fold(X_tr, y_tr, X_te, y_te, fold_num):
    ...
    model = TradingLSTM(n_feat, LSTM_V2_HIDDEN, LSTM_V2_LAYERS, LSTM_V2_DROPOUT).to(DEVICE)
    ...
    for epoch in range(1, LSTM_EPOCHS + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            logger.info(
                f"  Fold {fold_num} Ep {epoch:>3} | val={f1:.4f} tr={f1_tr:.4f} "
                f"gap={f1_tr-f1:+.3f} | Best={best_f1:.4f}"
            )
...
def retrain_final(X_all, y_all, n_epochs):
    ...
    loader = DataLoader(ds, batch_size=max(32, LSTM_BATCH_SIZE // 4), shuffle=True, num_workers=0)
    n_feat = X_sc.shape[2]
    model = TradingLSTM(n_feat, LSTM_V2_HIDDEN, LSTM_V2_LAYERS, LSTM_V2_DROPOUT).to(DEVICE)
    criterion = FocalLoss(alpha=focal_alpha(y_all), gamma=FOCAL_CFG["focal_gamma"])
    optimizer = torch.optim.Adam(
        model.parameters(), lr=LSTM_V2_LR,
        weight_decay=LSTM_V2_WEIGHT_DECAY, foreach=False,
    )
    ...
    try:
        ...
        if "memory" not in str(exc).lower():
            raise
        logger.warning(f"GPU OOM, falling back to CPU: {exc}")
        model = model.cpu()
        criterion = FocalLoss(alpha=focal_alpha(y_all).cpu(), gamma=FOCAL_CFG["focal_gamma"])
        optimizer = torch.optim.Adam(
            model.parameters(), lr=LSTM_V2_LR, weight_decay=LSTM_V2_WEIGHT_DECAY, foreach=False,
        )
        model.train()
        for epoch in range(1, n_epochs + 1):
            for xb, yb in loader:
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  LSTM IC32 CANDIDATE DOMAIN v3 -- {RUN_NAME}")
    print(f"  Partner : {LGBM_RUN} | Label: swing H4")
    print(f"  Features: {len(LSTM_FEATURES)} (avg IC ~0.055 vs v2 ~0.032)")
    print(f"  SEQ_LEN : {SEQ_LEN} | PURGE: {PURGE_GAP} | Folds: {N_FOLDS}")
    print(f"  Sample  : LGBM candidate bars (p2>={CAND_THR} OR p0>={CAND_THR})")
    ...
    logger.info(f"Saved OOF: {oof_has.sum():,} bars")

    avg_epochs  = int(np.median([m.get("best_epoch", 20) for m in all_metrics]))
    final_epochs = max(15, min(avg_epochs + 5, LSTM_EPOCHS))
    ...
    print(f"  OOF bars  : {oof_has.sum():,} / {len(y):,}")
    for m in all_metrics:
        print(
            f"  Fold {m['fold']}: F1={m['val_f1']:.4f} "
            f"S={m['f1_SHORT']:.3f} F={m['f1_FLAT']:.3f} L={m['f1_LONG']:.3f}"
        )
