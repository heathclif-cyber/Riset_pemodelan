"""
pipeline/05g_train_lstm_candidate_v3_seq36.py
14 fitur tinggi-IC, SEQ_LEN=48. Antara seq24 dan seq72.
"""
...
logger = setup_logger("05g_train_lstm_candidate_v3_seq36")
DEVICE = get_lstm_device()

RUN_NAME  = "ic32_lstm_candidate_v3_seq36"
...
def fit_scaler(X):
    n, s, f = X.shape
    sc = RobustScaler(); sc.fit(X.reshape(-1, f)); return sc
...
def load_data(coins, lgbm_oof):
    ...
    if not X_seqs: raise ValueError("No sequences.")
...
def retrain_final(X_all, y_all, n_epochs):
    ...
    model = TradingLSTM(X_sc.shape[2], LSTM_V2_HIDDEN, LSTM_V2_LAYERS, LSTM_V2_DROPOUT).to(DEVICE)
    ...
    try:
        ...
        logger.warning(f"GPU OOM, CPU fallback: {exc}")
        model = model.cpu()
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  LSTM IC32 CANDIDATE v3 SEQ36 -- {RUN_NAME}")
