...
# hmm_regime_enc dikeluarkan dari NON_FEATURE_COLS agar bisa dipakai sebagai fitur
...
def load_symbols(coins: list[str]) -> pd.DataFrame:
    ...
    for sym in coins:
        ...
        # Merge HMM regime labels jika tersedia (walk-forward safe, generated by 03e)
        regime_path = LABEL_DIR / f"{sym}_regime_h1.parquet"
        if regime_path.exists():
            try:
                reg = pd.read_parquet(regime_path)
                # drop kolom lama jika sudah ada (dari engineer run sebelumnya)
                if "hmm_regime_enc" in df.columns:
                    df = df.drop(columns=["hmm_regime_enc"])
                df = df.join(reg[["hmm_regime_enc"]], how="left")
                df["hmm_regime_enc"] = df["hmm_regime_enc"].fillna(1).astype("int32")
            except Exception as e:
                logger.warning(f"{sym}: regime merge gagal ({e}), skip")
...
def walk_forward_cv(X: pd.DataFrame, y: pd.Series, params: dict, n_splits: int = N_FOLDS, purge: int = PURGE_GAP_BARS):
    ...
    for fold, (train_idx, val_idx) in enumerate(folds, 1):
        ...
        # === Training predictions (untuk deteksi overfitting) ===
        y_train_pred = model.predict(X_tr)
        train_f1_macro = float(f1_score(y_tr, y_train_pred, average="macro", zero_division=0))
        train_f1_per   = f1_score(y_tr, y_train_pred, average=None, zero_division=0, labels=[0, 1, 2])
        ...
        metrics = {
            ...
            "train_f1_weighted": round(float(f1_score(y_tr, y_train_pred, average="weighted", zero_division=0)), 4),
        ...
        # Log validation + gap (untuk cepat lihat overfitting)
        val_f1 = f1_macro
        gap = train_f1_macro - val_f1
        logger.info(
            f"  Fold {fold}: Train F1={train_f1_macro:.4f} | Val F1={val_f1:.4f} | Gap={gap:+.4f} | "
            f"Val Acc={acc:.4f} | LONG={f1_per[2]:.4f} SHORT={f1_per[0]:.4f} FLAT={f1_per[1]:.4f}"
        )
...
def parse_args():
    ...
    parser.add_argument("--feature-cols", default=None,
                        help="Path ke JSON berisi list fitur (misal: pruned 87 fitur). "
                             "Default: semua fitur dari data.")
...
def main():
    ...
    if args.feature_cols:
        ...
        logger.info(f"Feature cols dari file: {len(feat_cols)} fitur ({args.feature_cols})")
    else:
