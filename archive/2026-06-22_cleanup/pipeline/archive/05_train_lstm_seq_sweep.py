"""
pipeline/05_train_lstm_seq_sweep.py — LSTM Seq_Len Sweep Training
Versi modifikasi dari 05_train_lstm.py yang menerima --seq-len via CLI.

Jalankan di RunPod:
  python pipeline/05_train_lstm_seq_sweep.py --seq-len 32 --all
  python pipeline/05_train_lstm_seq_sweep.py --seq-len 128 --all

Output: models/runs/lstm_seq{N}/
  - lstm.pt
  - lstm_scaler.pkl
...
logger = setup_logger("05_train_lstm_seq_sweep")
...
def load_symbols(coins: list) -> pd.DataFrame:
    frames = []
    for sym in coins:
...
def train_fold(X_tr, y_tr, X_te, y_te, fold_num, seq_len):
    logger.info(f"[Fold {fold_num}] Train={len(X_tr):,} | Test={len(X_te):,} | seq_len={seq_len}")

    scaler = StandardScaler()
    X_tr_sc = scaler.fit_transform(X_tr)
    X_te_sc = scaler.transform(X_te)

    train_ds = SequenceDataset(X_tr_sc, y_tr, seq_len)
    test_ds = SequenceDataset(X_te_sc, y_te, seq_len)
...
def retrain_final_lstm(X_all, y_all, final_epochs, scaler, seq_len):
    logger.info(f"Retraining final LSTM on 100% data ({len(X_all):,} samples, seq_len={seq_len}, epochs={final_epochs})...")
    X_all_sc = scaler.transform(X_all)

    train_ds = SequenceDataset(X_all_sc, y_all, seq_len)
    sampler = build_sampler(train_ds.get_labels())
    train_loader = DataLoader(train_ds, batch_size=LSTM_BATCH_SIZE,
                              sampler=sampler, num_workers=4, pin_memory=True)
...
def parse_args():
    parser = argparse.ArgumentParser(description="LSTM Seq_Len Sweep Training")
    parser.add_argument("--all", action="store_true", help="Use ALL_COINS instead of TRAINING_COINS")
    parser.add_argument("--seq-len", type=int, default=32, help="LSTM sequence length to train with")
...
def main():
    ...
    run_id = f"lstm_seq{seq_len}"
    run_dir = MODEL_DIR / "runs" / run_id
    run_dir.mkdir(parents=True, exist_ok=True)

    # Add file handler
    fh = logging.FileHandler(LOG_DIR / f"lstm_seq{seq_len}.log", encoding="utf-8")
    fh.setFormatter(logging.Formatter("%(asctime)s | %(levelname)-8s | %(name)s | %(message)s", datefmt="%Y-%m-%d %H:%M:%S"))
    fh.setLevel(logging.DEBUG)
    logger.addHandler(fh)

    logger.info(f"Device: {DEVICE} | Run: {run_id} | seq_len: {seq_len}")
    ...
    for fold_num, (tr_pos, te_pos) in enumerate(folds, 1):
        model, scaler, metrics = train_fold(
            X[tr_pos], y[tr_pos], X[te_pos], y[te_pos], fold_num, seq_len
        )
        all_metrics.append(metrics)
        if metrics["f1_macro"] > best_f1:
            best_f1, best_model, best_scaler, best_fold = metrics["f1_macro"], model, scaler, fold_num

    # Final scaler on full dataset
    scaler_path = run_dir / "lstm_scaler.pkl"
    logger.info(f"Fitting final scaler on {len(X):,} samples...")
    ...
    logger.info(f"CV complete. avg_best_epoch={avg_best_epoch} | Best Fold={best_fold} (F1={best_f1:.4f})")

    final_model = retrain_final_lstm(X, y, avg_best_epoch, final_scaler, seq_len)

    # Save model
    lstm_path = run_dir / "lstm.pt"
    save_lstm(final_model, lstm_path)
    logger.info(f"Model saved -> {lstm_path}")
    ...
    sep = "=" * 65
    print(f"\n{sep}")
    print(f"  LSTM SEQ_LEN={seq_len} TRAINING SELESAI — {run_id}")
    print(f"{sep}")
