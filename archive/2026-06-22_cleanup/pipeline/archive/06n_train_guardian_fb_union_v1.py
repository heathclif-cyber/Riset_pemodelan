"""
pipeline/06n_train_guardian_fb_union_v1.py
Guardian Opsi B: union flatboost_v2 + feature_cols_v2 + 7 dynamic

Static features = union(27 fb_v2, 33 feature_cols_v2) -- tidak duplikat
Dynamic features = 7 trade-progress features
Total = 36 static + 7 dynamic = 43 (max, tergantung ketersediaan kolom)

Labeling: sama persis dengan profit_v1 (profit-only)
Output: models/runs/tb_guardian_fb_union_v1/
"""
...
logger = setup_logger("06n_guardian_fb_union_v1")

RUN_NAME  = "tb_guardian_fb_union_v1"
...
# ── Feature config: Opsi B = union kedua set ─────────────────────────────────
with open(MODEL_DIR / "runs" / FB_RUN / f"{FB_RUN}_features.json") as f:
    fb_static = json.load(f)                             # 27 fitur

with open(MODEL_DIR / "feature_cols_v2.json") as f:
    v2_static = json.load(f)                             # 33 fitur

# Union: pertahankan urutan fb_static dulu, tambah v2 yang belum ada
seen = set(fb_static)
union_static = list(fb_static)
for f in v2_static:
    if f not in seen:
        union_static.append(f)
        seen.add(f)

STATIC_FEATS = union_static
...
logger.info(f"Opsi B: {len(STATIC_FEATS)} static (union) + {len(DYNAMIC_FEATS)} dynamic = {len(ALL_FEATS)} total")
logger.info(f"  fb_v2={len(fb_static)}, v2={len(v2_static)}, union={len(union_static)}")
...
def main():
    print(f"\n{'='*65}")
    print(f"  Guardian Opsi B Training  [tb_guardian_fb_union_v1]")
    print(f"  Static: {len(STATIC_FEATS)} fitur (union fb_v2 + feature_cols_v2)")
    print(f"    fb_v2={len(fb_static)}, v2={len(v2_static)}, union={len(STATIC_FEATS)}")
    print(f"  Dynamic: 7 fitur trade-progress")
    print(f"  Total : {len(ALL_FEATS)} fitur (max, tergantung kolom tersedia)")
    ...
    static_used = [c for c in feat_cols if c not in set(DYNAMIC_FEATS)]

    meta = {
        "run_name":       RUN_NAME,
        "trained_at":     datetime.now().isoformat(),
        "variant":        "Opsi B: union(27 fb_v2, 33 v2) static + 7 dynamic",
        "n_static_union": len(STATIC_FEATS),
        "n_static_used":  len(static_used),
    ...
    print(f"  Saved to {OUT_DIR} ({len(feat_cols)} features used)")
