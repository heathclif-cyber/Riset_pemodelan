"""
Diagnostic holdout: top-1 fusion candidate vs production baseline.

BUKAN grid holdout -- konfirmasi sekali setelah OOF sweep.
Entry frozen B-dir-combined; hanya cascade params yang berbeda.

Usage:
  python pipeline/07c_holdout_ic32_lstm_fusion_diag.py
"""
import json
import sys
import warnings
...
from pipeline.ic32_fusion_shared import (
    IC32_DIR, build_per_bar_thresholds, load_b_dir_hmm_cfg, load_production_defaults,
)
from config import (
...
logger = setup_logger("07c_lstm_fusion_holdout")
HOLDOUT_LABEL_DIR = HOLDOUT_DIR / "labeled"
STAGE2_OUT = IC32_DIR / "ic32_lstm_fusion_stage2_pipeline.json"
OUT_JSON = IC32_DIR / "holdout_lstm_fusion_diag_apr_jun26.json"
FLOW_MOM_WINDOW = 3
...
def _apply_cascade_cfg(cfg: dict) -> dict:
    ...
    return {"conf_entry": project_config.CONFIDENCE_THRESHOLD_ENTRY, "sl_trigger_mode": sl_mode}
...
def _run_holdout(sym: str, cfg: dict, hmm_cfg: dict, feat_cols: list, lstm_feats: list,
                 lgbm, lstm, lstm_scaler, gdn: dict, live_cfg: dict) -> list:
    ...
    y_pred, confidence = hierarchical_predict(
        ...
        model_dir=IC32_DIR, lstm_feat_cols=lstm_feats,
...
def main():
    ...
    if not best:
        raise RuntimeError("No best candidate from stage2")

    prod = load_production_defaults()
    baseline_cfg = {"fusion": "baseline", "label": "baseline_production", **prod}
    candidate_cfg = {k: best[k] for k in best if k not in (
        "n", "wr", "pnl", "ppt", "pf", "sl_pct", "delta_ppt", "delta_n", "passes_gate",
        "stage1_rank_score",
    )}
    ...
    results = {}
    for name, cfg in [("baseline_production", baseline_cfg), (candidate_cfg["label"], candidate_cfg)]:
        live_cfg = _apply_cascade_cfg(cfg)
        trades = []
        for sym in ALL_COINS:
            trades.extend(_run_holdout(sym, cfg, hmm_cfg, feat_cols, lstm_feats,
                                       lgbm, lstm, lstm_scaler, gdn, live_cfg))
        results[name] = _scorecard(trades)
        logger.info(f"{name}: {results[name]}")

    b = results["baseline_production"]
    c = results[candidate_cfg["label"]]
    out = {
        "methodology": "holdout_diagnostic_once",
        "period": "Apr-Jun 2026",
        "stage2_source": str(STAGE2_OUT),
        "candidate": candidate_cfg["label"],
        "baseline": b,
        "candidate_scorecard": c,
    ...
    print("\n=== Holdout LSTM fusion diagnostic ===")
    print(f"{'variant':<40} {'trades':>7} {'WR%':>7} {'PPT':>8} {'PF':>6}")
    for name, sc in results.items():
        if sc.get("trades", 0) == 0:
            continue
        print(f"{name:<40} {sc['trades']:>7} {sc['win_rate']:>6.1f}% "
