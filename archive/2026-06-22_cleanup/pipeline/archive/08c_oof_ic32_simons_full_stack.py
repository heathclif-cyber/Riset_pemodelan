"""
pipeline/08c_oof_ic32_simons_full_stack.py — Full-Stack OOF with frozen Simons Config C

Same stack as 08_oof_ic32_full_stack (LSTM + FLIP + Guardian) but LGBM entry
thresholds come from HMM per-state sweep winner (Config C), not fixed 0.69/0.59.

Prerequisite:
  pipeline/04_train_lgbm_ic32_genuine_oof.py
  pipeline/05e_hmm_threshold_sweep_ic32.py

Jalankan:
  python pipeline/08c_oof_ic32_simons_full_stack.py
"""
import json
import sys
import warnings
...
logger = setup_logger("08c_ic32_simons_oof")
RUN_DIR = MODEL_DIR / "runs" / "ic32_regime_v1"
OOF_PATH = RUN_DIR / "oof_predictions.parquet"
SIMONS_PATH = RUN_DIR / "hmm_threshold_ic32_simons.json"
INF_CFG_PATH = MODEL_DIR / "inference_config.json"
BASELINE_PATH = RUN_DIR / "oof_full_stack_scorecard.json"
...
def _load_simons_config() -> dict:
    with open(SIMONS_PATH, encoding="utf-8") as f:
        data = json.load(f)
    raw = data["best_config"]
...
def backtest_coin_oof(sym: str, oof_all: pd.DataFrame, feat_cols: list,
    ...
    thr_long_arr, thr_short_arr = _build_per_bar_thresholds(hmm_enc, hmm_cfg)
    ...
    y_pred, confidence = hierarchical_predict(
        ...
        per_bar_thr_long=thr_long_arr,
        per_bar_thr_short=thr_short_arr,
    )
...
def _aggregate_trades(all_trades: list) -> dict:
    ...
    short_long_ratio = len(short_trades) / len(long_trades) if long_trades else 0
    ...
    return {
        ...
        "short_long_ratio": round(short_long_ratio, 3),
...
def main():
    ...
    if not SIMONS_PATH.exists():
        raise FileNotFoundError(
            f"{SIMONS_PATH} missing — run pipeline/05e_hmm_threshold_sweep_ic32.py first"
        )

    live_cfg = _apply_live_config()
    hmm_cfg = _load_simons_config()
    ...
    sep = "=" * 80
    print(f"\n{sep}")
    print("  ic32_regime_v1 — FULL-STACK OOF (Simons Config C HMM thresholds)")
    print(f"  Period  : training until {TRAIN_CUTOFF_DATE.date()}")
    print(f"  HMM thr : {hmm_cfg}")
    print(f"  CONF    : {live_cfg['conf_entry']} | Guardian: {gdn_name} exit={live_cfg['gdn_exit']}")
    print(f"  FLIP    : {live_cfg['flip']} | Production baseline thr: {live_cfg['thr_long']}/{live_cfg['thr_short']}")
    print(f"{sep}\n")
    ...
    print(f"\n{sep}")
    print("  OOF SCORECARD — ic32 full stack + Simons Config C")
    print(f"{sep}")
    print(f"  Total Trades     : {agg['total_trades']:,}")
    print(f"  Trades/bulan     : {agg['trades_per_month']:.0f}")
    print(f"  Win Rate         : {agg['win_rate']:.1f}%")
    print(f"  LONG WR          : {agg['long_wr']:.1f}% ({agg['long_pct']:.1f}% of trades)")
    print(f"  SHORT WR         : {agg['short_wr']:.1f}%")
    print(f"  SHORT/LONG ratio : {agg['short_long_ratio']:.2f}")
    print(f"  Net PnL          : ${agg['total_pnl']:+.2f}")
    print(f"  PnL/trade        : ${agg['pnl_per_trade']:+.4f}")
    print(f"  Profit Factor    : {agg['profit_factor']:.3f}")
    print(f"  Guardian exits   : {agg['guardian_exit_pct']:.1f}%")

    baseline_agg = None
    if BASELINE_PATH.exists():
        with open(BASELINE_PATH, encoding="utf-8") as f:
            baseline_agg = json.load(f)["aggregate"]
        print(f"\n  vs Baseline full-stack (fixed 0.69/0.59):")
        print(f"    Baseline: {baseline_agg['total_trades']} trades | WR {baseline_agg['win_rate']:.1f}% | "
              f"PF {baseline_agg['profit_factor']:.2f} | PPT ${baseline_agg['pnl_per_trade']:.4f}")
        print(f"    Simons C: {agg['total_trades']} trades | WR {agg['win_rate']:.1f}% | "
              f"PF {agg['profit_factor']:.2f} | PPT ${agg['pnl_per_trade']:.4f}")
        d_trades = agg["total_trades"] - baseline_agg["total_trades"]
        d_wr = agg["win_rate"] - baseline_agg["win_rate"]
        d_pf = agg["profit_factor"] - baseline_agg["profit_factor"]
        d_ppt = agg["pnl_per_trade"] - baseline_agg["pnl_per_trade"]
        print(f"    Delta  : trades {d_trades:+,} | WR {d_wr:+.1f}pp | PF {d_pf:+.3f} | PPT ${d_ppt:+.4f}")

    out = {
        "meta": {
            "run": "ic32_regime_v1",
            "methodology": "genuine_oof_full_stack_simons_config_c",
            "period": f"2020-01-01 to {TRAIN_CUTOFF_DATE.date()}",
            "simons_config_source": str(SIMONS_PATH),
            "live_config_source": str(INF_CFG_PATH),
            "guardian_model": gdn_name,
            "dynamic_sizing": False,
            "generated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        },
        "hmm_per_state_thresholds": {str(k): list(v) for k, v in _load_simons_config().items()},
        "live_params": live_cfg,
        "aggregate": agg,
        "vs_baseline_full_stack": (
            {
                "baseline_trades": baseline_agg["total_trades"],
                "baseline_wr": baseline_agg["win_rate"],
                "baseline_pf": baseline_agg["profit_factor"],
                "baseline_ppt": baseline_agg["pnl_per_trade"],
                "delta_trades": agg["total_trades"] - baseline_agg["total_trades"],
                "delta_wr_pp": round(agg["win_rate"] - baseline_agg["win_rate"], 2),
                "delta_pf": round(agg["profit_factor"] - baseline_agg["profit_factor"], 3),
                "delta_ppt": round(agg["pnl_per_trade"] - baseline_agg["pnl_per_trade"], 4),
            }
            if baseline_agg
            else None
        ),
    ...
    out_path = RUN_DIR / "oof_simons_full_stack_scorecard.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2)

    trades_path = RUN_DIR / "oof_simons_trade_history.csv"
