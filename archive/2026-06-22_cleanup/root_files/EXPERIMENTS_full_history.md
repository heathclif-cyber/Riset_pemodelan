...
## 2026-06-22 — HMM Retrain dengan BTC Cross-Asset Features (ic32_regime_v3)

**Status**: COMPLETED

### Hipotesis
HMM per-koin tanpa konteks BTC membuat label regime tidak mencerminkan kondisi
market sesungguhnya. Menambahkan btc_ret_h4 dan btc_mom_h4 sebagai fitur ke-5 dan ke-6
memberi sinyal cross-asset yang membedakan "altcoin trending UP karena BTC naik"
vs "altcoin trending UP sendiri". Hipotesis: label regime lebih informatif untuk LGBM.

Catatan metodologi: analisis eksperimen sebelumnya (WR gap, direction filter, threshold sweep)
TIDAK VALID — ditemukan many-to-many join bug yang menginflatkan 800k rows menjadi 13M.
Retrain ini dimotivasi oleh economic reasoning, bukan angka eksperimen sebelumnya.

### Yang Diubah
- `core/regime.py`: `_build_hmm_features` + 2 fitur (btc_ret_h4, btc_mom_h4)
- `pipeline/03e_regime_hmm.py`: load BTC H4, pass ke generate_oof_regime_labels
- Full retrain: 03e_regime_hmm -> 04_train_lgbm -> 06_train_guardian
- Model run dir: models/runs/ic32_regime_v3/ (pisah dari v2 agar v2 bisa fallback)

### Target
- Bandingkan OOF WR dan PF vs ic32_regime_v2 (baseline: WR 56.64%, PF 1.372)
- Holdout TIDAK dibuka — hanya OOF comparison
- Kriteria adopt: OOF WR >= 56.64% DAN PF >= 1.372 DAN trades >= 80% v2

### Script
- `core/regime.py` — modifikasi _build_hmm_features (+ btc_h4 parameter)
- `pipeline/03e_regime_hmm.py` — pass btc_h4
- `pipeline/04_train_lgbm_ic32_regime_v2.py` — RUN_NAME=ic32_regime_v3
- `pipeline/06_train_guardian_ic32_regime_v2.py` — LGBM_RUN_NAME=ic32_regime_v3

### Hasil CV OOF

| Model | WR | Trades | PnL | PPT |
|-------|-----|--------|-----|-----|
| ic32_regime_v2 (baseline) | 64.7% | 12,861 | $3,632 | $0.2824 |
| ic32_regime_v3 (BTC feat) | **64.9%** | **13,471** | **$3,768** | $0.2797 |
| Delta | +0.2pp | +610 | +$136 | -0.0027 |

| Guardian | F1 | Logloss |
|----------|-----|---------|
| ic32_rv2_guard (v2) | 0.8662 | 0.1695 |
| ic32_rv3_guard (v3) | 0.8624 | 0.1733 |

### Kesimpulan
v3 lebih baik dari v2 di LGBM OOF: WR +0.2pp, trades +610, PnL +$136 total.
Namun perbedaan sangat kecil (di bawah noise CV ±0.0112 F1).
Guardian v3 sedikit LEBIH BURUK dari v2 (F1 -0.0038, logloss +0.0038).

Kriteria adopt (semua harus terpenuhi):
- WR >= 64.7% (v2): v3 = 64.9% PASS
- Trades >= 80% v2: v3 = 13,471 (105%) PASS
- Holdout BELUM DIEVALUASI — perlu holdout backtest untuk keputusan final

**Keputusan**: holdout ic32_regime_v3 belum dijalankan. Jika dijalankan harus sekali saja.
Pertimbangkan apakah delta yang kecil (+$136 OOF total) cukup justifikasi untuk buka holdout.

---

## 2026-06-22 — HMM Feature Selection (regime detector improvement)

**Status**: COMPLETED (join bug found, hasil tidak valid)

### Hipotesis
HMM saat ini pakai 4 fitur per-koin (return_1bar, volatility_24, momentum_48, volume_ratio).
Diduga feature set ini kurang informatif karena:
1. `momentum_48` = window 8 hari H4 → terlalu lambat mendeteksi regime baru (lag 4-8 hari)
2. HMM di-fit per-koin terpisah → tidak ada konteks cross-asset (ARBU bisa TRENDING_UP saat BTC TRENDING_DOWN)
3. Belum divalidasi: apakah WR LGBM OOF benar-benar berbeda signifikan antar state?

### Yang Diubah
- Tambah fitur cross-asset ke HMM: BTC log-return H4 dan/atau universe index
- Test Parkinson/GK realized vol vs rolling std saat ini
- Test Correlation Dispersion sebagai fitur ke-5
- Evaluasi global HMM (satu model semua koin) vs per-koin saat ini

### Target
- WR LGBM OOF per state harus berbeda signifikan (gap >= 5% antar state terburuk vs terbaik)
- Baseline: analisis WR per state dari OOF ic32_regime_v2 yang ada sekarang
- Jika HMM baru lebih informatif → retrain LGBM dengan regime label baru

### Baseline (terukur)
- Gap WR antar state: **1.0%** (44.7%-45.7%) — HMM saat ini tidak informatif
- HMM internal valid: persistence diagonal >0.90, means masuk akal secara ekonomi
- Masalah: fitur per-koin tidak tangkap konteks cross-asset

### Desain Eksperimen: Feature Set Sweep
Tanpa retrain LGBM — gunakan OOF predictions yang sudah ada.
Untuk tiap feature set: fit HMM baru per koin → label OOF bars → hitung WR gap.
Metric utama: WR gap (max_state_WR - min_state_WR) pada OOF directional predictions.

Feature sets yang diuji:
- V0 (baseline)  : [ret_1bar, vol_24, mom_48, vol_ratio]
- V1             : V0 + btc_ret_h4
- V2             : V0 + btc_ret_h4 + btc_mom_48
- V3             : Parkinson vol + ret + mom + btc_ret
- V4             : V0 + universe_index (avg return 21 koin)
- V5             : V0 + universe_index + corr_dispersion_20
- V6             : V5 + btc_ret

### Hasil Sweep (2026-06-22)

| Version | Fitur Tambahan | WR Gap |
|---------|---------------|--------|
| V0 baseline | - | 0.28% |
| V1 | + btc_ret | 0.93% |
| V2 | + btc_ret + btc_mom | **1.56%** |
| V3 | Parkinson vol + btc_ret | 0.89% |
| V4 | + universe_index | 0.36% |
| V5 | + universe + corr_disp | 0.62% |
| V6 | V5 + btc_ret | 0.36% |

**Status**: COMPLETED — KESIMPULAN NEGATIF

### Kesimpulan
Semua 7 feature set menghasilkan WR gap < 2% antar state HMM.
V2 terbaik (1.56%) tapi masih jauh dari threshold minimal 5%.
HMM tidak informatif terlepas dari feature set yang dipakai.

Root cause: LGBM sudah menangkap regime information dari fiturnya sendiri
(RSI, EMA trend, vol_spike_zscore, h4_trend, momentum). HMM menjadi redundan.

**Langkah selanjutnya yang dipertimbangkan:**
Direction filter test dilakukan — lihat hasil di bawah.

### Hasil Direction Filter Test (2026-06-22)

| Version | TRENDING_DOWN delta | TRENDING_UP delta | Max delta |
|---------|--------------------|--------------------|-----------|
| V2 (ret+vol+mom+vr+btc_ret+btc_mom) | SHORT-LONG = **+9.24%** | LONG-SHORT = **+5.61%** | **9.24%** |
| V3 (Parkinson+btc_ret) | +7.55% | +5.98% | 7.55% |
| V6 (V5+btc_ret) | +4.88% | +6.32% | 6.32% |
| V0 baseline | +4.49% | +2.66% | 4.49% |

**V2 TERBAIK**: di TRENDING_DOWN, SHORT WR=49.6% vs LONG WR=40.3% (delta 9.24%)
Bahkan baseline V0 pun berguna sebagai filter (delta 4.49%).

**Kesimpulan revised**: HMM TIDAK berguna sebagai WR predictor, tapi SANGAT berguna
sebagai direction filter. State menentukan arah trade mana yang lebih akurat.

### Hasil OOF Filter Simulation (2026-06-22)

| | Trades | WR | PF | Net ATR |
|--|--------|----|----|---------|
| Baseline (no filter) | 375,760 | 62.11% | 2.115 | 49,206 |
| Filter A (blok counter-trend) | 305,988 | 62.71% | 2.170 | 41,385 |

Filter menghapus 18.6% trade → WR +0.61%, PF +0.055, tapi net ATR -7,821.

**Kesimpulan final**: Hard filter TIDAK layak — mengurangi total PnL.
Trade yang diblok punya EV positif (~0.10 ATR/trade), hanya lebih rendah dari yang dipertahankan.

**Rekomendasi**: Gunakan sebagai SOFT threshold adjustment per state.
Sweep threshold per state pada OOF — lihat hasil di bawah.

### Per-State Threshold Sweep (2026-06-22)
Script: tools/_tmp_hmm_threshold_sweep.py
Metric: **PF** (quality), constraint: trades 70%–130% baseline per state

| State | long_thr | short_thr | PF lama | PF baru | Delta PF |
|-------|----------|-----------|---------|---------|----------|
| TRENDING_DOWN | 0.85 | 0.70 | 2.220 | 2.378 | +0.158 |
| RANGING_LOW_VOL | 0.77 | 0.72 | 1.938 | 2.072 | +0.134 |
| RANGING_HIGH_VOL | 0.72 | 0.77 | 2.185 | 2.425 | +0.240 |
| TRENDING_UP | 0.77 | 0.82 | 2.820 | 3.160 | +0.340 |

Pipeline overall: trades -23% (343k→263k), net EV -7.3k ATR, tapi per-trade EV +10.3%
(0.135 → 0.149 ATR/trade).

**Temuan kunci**:
- TRENDING_DOWN: long_thr 0.75→0.85, WR_LONG naik 60.5%→69.2% (selaras direction filter hypothesis)
- RANGING_HIGH_VOL: asimetrik — LONG lebih mudah (0.75→0.72), SHORT lebih sulit (0.70→0.77)
- TRENDING_UP: short_thr naik 0.80→0.82

**Metodologi**: threshold sweep di OOF valid — sama dengan pipeline/04 threshold sweep original.
Implementasi hanya ubah inference_config.json, tidak perlu retrain model.

**Keputusan**: pending user approval untuk deploy ke inference_config.json

---

## 2026-06-22 — DEPLOY ic32_regime_v2 → LIVE (gantikan alpha v1)

**Status**: DEPLOYED (swint commit 4b6e15d, VPS 139.180.157.176, app_revision aktif)

### Yang dideploy
Mengganti alpha live `ic32_regime_v1` dengan stack v2 (parity riset↔live). Tujuan user: live
menjalankan metodologi v2 yang SAMA dengan backtest, agar tes "live=riset" beberapa hari ke depan valid.

- **inference_config.json**: entry **LGBM-murni** (`lstm_confirmation_enabled=false`,
  `regime_alignment.enabled=false`, `cascade.dynamic_threshold_enabled=false`), HMM per_state thr
  long=0.75 / short=0.70 (state3 TREND_UP short=0.80). Guardian v3mom exit_thr=0.90, min_hold=2,
  `momentum_floor_frac=0.7`. **pyramiding + VCB OFF** (samakan backtest pipeline/07).
- **guardian_service.py** (sumber `tools/production/`): ditulis ulang utk fitur v3mom — `lgbm_entry_conf`
  (dari `trade.signal.confidence`), `mfe_atr_ratio`; replika `_assemble_guardian_row`.
- **paper_trading.py** (swint): trailing floor 0.7 + MFE high/low intrabar (match `core/evaluator.py`).
- **inference.py** (swint): gate `momentum_reduce` di belakang `dynamic_threshold_enabled`.
- **deploy_model.py**: ACTIVE_STACK guardian → `ic32_rv2_guard_oof_v3mom`.
- **data_service.py** (swint, fix sesi lalu ikut terdeploy): `btc_h1_return` log return + hapus CVD clamp.

### Verifikasi pasca-deploy
- Config VPS: ✅ model_version ic32_regime_v2, lstm off, guardian v3mom, floor 0.7, pyr/vcb off.
- File v2/v3mom ✅ ada di VPS. Smoke test load+predict (guardian 30-feat, lgbm 33-feat) ✅ lokal.
- `/api/health` ✅ app_revision 4b6e15d, scheduler running.
- **Belum**: siklus `generate_signals` live pertama (HH:05) — konfirmasi v2 load tanpa KeyError di
  feature pipeline live.

### Divergensi sisa (akan diukur user via holdout test)
- Sizing live tiered vs backtest fixed $5 → beda magnitudo PnL/trade, bukan arah/WR.
- `model_registry.json` VPS masih tampil v1 (kosmetik — artefak diambil dari `inference_config.models`).
- HMM regime: pakai HMM pkl per-coin yang ter-deploy (diasumsikan = riset, pre-existing).

### Cara verifikasi (oleh user, beberapa hari ke depan)
Bandingkan signal/trade live (via `tools/live_db_bridge.py` + `trade_analyzer.py`) vs ekspektasi
backtest v2 pada periode yang sama: nilai fitur, arah, confidence per bar harus match.

---

## 2026-06-22 — OOF Sweep: GUARDIAN_MOMENTUM_FLOOR_FRAC → 0.70

**Status**: COMPLETED

### Hipotesis

Trailing floor 0.50 terlalu protektif (50.7% floor exit, avg_win $0.393 < V1 $0.441). OOF sweep untuk
mencari nilai optimal floor_frac yang memaksimalkan PnL/PF tanpa korbankan WR (Aturan 1 — hanya OOF,
bukan holdout).

### Yang Diubah

- `GUARDIAN_MOMENTUM_FLOOR_FRAC` di `config.py`: 0.50 → **0.70**

### Hasil OOF Sweep (ic32_regime_v2 LGBM OOF + Guardian v3mom, 21 koin, 2020–2026-03)

| frac | trades | WR%   | PF    | PnL      | avg_win | floor% |
|------|--------|-------|-------|----------|---------|--------|
| 0.20 | 11888  | 65.74 | 3.364 | $3671    | $0.669  | 15.1%  |
| 0.35 | 12002  | 68.36 | 3.410 | $3705    | $0.639  | 28.4%  |
| 0.50 | 12362  | 70.98 | 3.444 | $3789    | $0.609  | 43.0%  |
| **0.70** | **12879** | **72.72** | **3.436** | **$3904** | **$0.588** | **53.6%** |
| 0.80 | 13004  | 72.99 | 3.364 | $3832    | $0.575  | 55.9%  |
| 0.90 | 13043  | 73.12 | 3.310 | $3754    | $0.564  | 56.5%  |

Hasil lengkap: `models/runs/ic32_rv2_guard_oof_v3mom/floor_frac_oof_sweep.csv`

### Kesimpulan

PnL puncak di **frac=0.70** ($3904, +$115 vs 0.50). PF hampir sama (3.436 vs 3.444 di 0.50).
Di atas 0.70: PF dan PnL keduanya turun (floor terlalu ketat, avg_loss membesar).
frac=0.70 = exit saat PnL pullback ke bawah 70% MFE — lebih protektif dari 0.50 (50% MFE),
lock lebih banyak dari puncak, cycling trade lebih cepat (+517 trade vs 0.50).
Config diupdate; sweep via `tools/sweep_floor_frac_oof.py`.

---

## 2026-06-22 — ic32_regime_v2 — Parity Audit: raw CVD stationarization + Guardian v3 momentum riding

**Status**: COMPLETED

### Hipotesis

Audit parity fitur Riset↔Live (swint_tradev2). Temuan: derived CVD (`cvd_slope_h4`, `cvd_momentum_adv`)
sudah scale-invariant (entri 2026-06-20), TAPI **raw `cvd` = `delta.cumsum()` masih path-dependent** —
masuk LGBM langsung (4.1% importance) + basis `whale_retail_divergence`. Live cumsum ~3000 bar tidak
akan pernah sama dengan training cumsum ~50k bar → arah/sinyal live menyimpang. Hipotesis: stationarisasi
raw `cvd` (rolling de-mean z-score) menghapus offset cumsum konstan → nilai identik live vs training.

Tambahan: Guardian sebaiknya **ride momentum** (long & short) bukan berhenti di TP — ride lebih optimal.

### Yang Diubah

- **`core/features.py`**: kolom ekspor `cvd` (yang masuk LGBM) di-overwrite jadi
  `(cvd - cvd.rolling(168).mean()) / cvd.rolling(168).std()`, clip ±5. Semua turunan cvd dihitung
  SEBELUM overwrite (pakai cumsum mentah) → slope/momentum/divergence/whale tak berubah.
- **Guardian v3 (`ic32_rv2_guard_oof_v3mom`)**: +3 static feat parity-proven (`hmm_regime_enc`,
  `log_ret_20`, `h4_trend`); **labeling regime-conditional** — tren kuat+flow searah longgarkan
  give-back (ride dalam), ranging kunci cepat; reinforce HOLD saat momentum kuat.
- **Trailing floor** (`GUARDIAN_MOMENTUM_FLOOR_FRAC=0.5` di config + evaluator): post-TP kunci min
  50% MFE sebagai hard safety. Outcome baru `GUARDIAN_MOMENTUM_FLOOR`.
- **swint_tradev2 data_service.py**: `btc_h1_return` `pct_change`→`np.log` (= training `log_ret_1`);
  hapus `_CVD_CLAMP_BOUNDS`/`_clamp_ood_features` (band-aid formula lama, tak perlu lagi).
- **Harness baru** `tools/audit_feature_parity.py`: uji full-history vs 3000-bar window.

### Hasil — Parity (harness, BTC/ETH/SOL, last 300 bar)

| Fitur | max_abs_diff full vs 3000-bar | corr |
|-------|------------------------------|------|
| `cvd` | 4.7e-11 (sebelumnya miliaran) | 1.0000 |
| SEMUA 42 fitur model | **0/126 pasangan divergen** (≤2.6e-8) | 1.0000 |

→ Live (window 3000) menghitung nilai fitur IDENTIK dengan training (histori penuh). Keluhan
"cvd dari 2020" terselesaikan untuk seluruh stack.

### Hasil — Holdout (Apr–20 Jun 2026, 21 koin, LGBM+Guardian v3mom+floor)

| Metrik | v2 BARU | V1 LIVE | v2-plain LAMA (old cvd) |
|--------|---------|---------|------------------------|
| Trades | 621 | 555 | 633 |
| WR | 71.82% | 68.83% | 68.56% |
| PF | 3.067 | 3.107 | 2.803 |
| PnL | $118.09 | $114.20 | $111.61 |
| Avg win | $0.393 | $0.441 | $0.400 |

Momentum riding: **61.8% trade ride lewat TP** (win-rate 95.3%, PnL $163.67). Exit dist:
`MOMENTUM_FLOOR 50.7%`, `GUARDIAN_EXIT 14.2%`, `MOMENTUM_EXIT 10.5%`, `LOSS 18.7%`, `TIMEOUT 5.9%`.
Guardian v3 CV: F1 macro 0.866, logloss 0.169, 138,917 samples, 30 fitur.

### Kesimpulan

Parity terbukti (0/126 divergen) — fix metodologi paling penting: sim/holdout kini mewakili live.
v2 baru mengalahkan V1 pada WR (+3 ppt) & PnL (+$3.9), PF nyaris setara (3.067 vs 3.107), trades >V1.
Momentum riding aktif & profitabel. CATATAN: trailing floor 0.5 dominan (50.7%) & avg-win ($0.393) <V1
($0.441) → floor cenderung protektif, membatasi runner. Untuk ride lebih jauh, turunkan
`GUARDIAN_MOMENTUM_FLOOR_FRAC` — WAJIB tuning di OOF (Aturan 1), bukan holdout. LSTM momentum di-stop
atas permintaan user; HMM tak terdampak (pakai H4 OHLCV saja). Belum deploy (perlu persetujuan eksplisit
user + wiring trailing-floor di live exit + retune exit_thr v3mom di OOF).

---

## 2026-06-20/21 — ic32_regime_v2 — Retrain Widyawardhana Fund: CVD Normalisasi + Pipeline Cleanup

**Status**: COMPLETED (OOF selesai, config freeze, holdout BELUM dijalankan)

### Hipotesis

`cvd_slope_h4` dan `cvd_momentum_adv` dinormalisasi oleh `abs(cumsum_CVD).rolling().mean()` — denominator yang tumbuh monoton seiring bertambahnya data historis. Ini menyebabkan:
1. 50% nilai training berkumpul di (-0.01, 0.01) → discriminative power rendah
2. Live fetch hanya 600 bar → denominator kecil → slope 30-100× lebih besar dari training → OOD (mitigated sementara via clamp)

Hipotesis: ganti normalisasi ke **z-score dari delta** (`delta / rolling_std(delta)`) → output stasioner, scale-free, dan reproducible di live tanpa clamp.

### Yang Diubah

- **`core/features.py` — `calc_cvd_divergence()`**: `cvd_slope_raw` dari `diff/abs_mean` → `diff/rolling_std(diff)`, clip ±3
- **`core/features.py` — `calc_cvd_hidden_divergence()`**: `cvd_momentum` dari `diff/abs_mean` → `diff/rolling_std(diff)`, clip ±3
- **CV method**: expanding walk-forward → **rolling walk-forward** (8-fold, purge=36 bar) — mitigasi bias data 2020 yang tidak relevan ke kondisi pasar terkini
- **Pipeline cleanup**: 180+ script eksperimen diarsipkan ke `pipeline/archive/`, sisa 13 script inti + rename ke ic32_regime_v2

### Target

- Metrik yang ingin dicapai: WR >= 66.5%, PF >= 2.69, trades >= 394 (80% × 492 trades ic32)
- Baseline: ic32_b_dir_combined — WR 66.5%, PF 2.69, 492 trades, +$0.340 PPT (holdout Apr–Jun 2026)

---

## ARSITEKTUR FINAL — ic32_regime_v2 (config freeze, siap holdout)

### Komponen Pipeline (urutan eksekusi)

```
03_engineer.py              → 33 fitur v3 per coin (termasuk CVD baru + hmm_regime_enc placeholder)
03e_regime_hmm.py           → HMM 4-state OOF walk-forward → hmm_regime_enc di parquet
04_train_lgbm_ic32_regime_v2.py → LGBM 8-fold rolling purged CV (purge=36) → OOF predictions
06_train_guardian_ic32_regime_v2.py → Guardian v1 trained pada OOF trades
07_holdout_ic32_regime_v2.py        → SEKALI, semua config sudah freeze
```

*LSTM (05_*): diputuskan TIDAK dipakai (lihat bagian LSTM di bawah).*

---

### Layer 1 — LGBM Entry Signal

| Parameter | Nilai |
|---|---|
| Model ID | `ic32.rv2.lgbm.33f.sw4.r8` |
| Universe | 21 coin (ic32), H1 OHLCV |
| Features | 33 fitur v3 (termasuk `hmm_regime_enc`, CVD z-score baru) |
| Label | Swing H4 (h4_swing_high / h4_swing_low), min_rr=1.5, min_tp=0.8 ATR |
| CV method | Rolling walk-forward, K=8, purge_gap=36 bar (MAX_HOLDING_BARS) |
| Training period | 2020-01-01 – 2026-04-01 |
| Run dir | `models/runs/ic32_regime_v2/` |

**Threshold entry (base):**
- LONG:  `p2 >= 0.75`
- SHORT: `p0 >= 0.70`

---

### Layer 2 — HMM Regime Filter (entry-level, bukan Guardian)

HMM 4-state (OOF walk-forward, `03e_regime_hmm.py`):

| State | Nama | SHORT default | SHORT jika state ini |
|---|---|---|---|
| 0 | TRENDING_DOWN | 0.70 | 0.70 (tidak berubah) |
| 1 | RANGING_LOW_VOL | 0.70 | 0.70 (tidak berubah) |
| 2 | RANGING_HIGH_VOL | 0.70 | 0.70 (tidak berubah) |
| 3 | TRENDING_UP | 0.70 | **0.80** (diperketat) |

**Rationale**: SHORT di TREND_UP (state 3) tanpa filter: WR=56.7%, PF=1.593 (jauh di bawah baseline).
Dengan `p0 >= 0.80`, hanya sinyal paling confident yang masuk — membuang sinyal lemah counter-trend.
LONG side: tidak ada filter (LONG di TREND_DN tetap bagus: WR=66.1%, PF=2.52).

---

### Layer 3 — Guardian Exit Controller

| Parameter | Nilai |
|---|---|
| Model ID | `ic32.rv2.guard.oof` (v1) |
| Run dir | `models/runs/ic32_rv2_guard_oof/` |
| CV F1 macro | 0.8853 |
| CV logloss | 0.1551 |
| Features | 25 (18 static + 7 dynamic) |
| Training domain | OOF trades only (TIDAK in-sample) |
| exit_thr | **0.90** |
| min_hold_bars | **2** |

**Guardian labels**: HOLD=0, PARTIAL_EXIT=1, FULL_EXIT=2
- Rule EXIT-2 di negative PnL **diizinkan** (v1 design, 38.4% EXIT di neg PnL) — ini intentional:
  Guardian belajar deteksi reversal sebelum SL hit. Membuang behavior ini (v2) justru memperburuk PnL.

---

### Hasil OOF — Progression (semua genuine OOF, bukan in-sample)

| Layer | Trades | WR | PF | PnL/T | PnL Total |
|---|---|---|---|---|---|
| LGBM standalone (0.75/0.70) | 12,992 | 64.20% | 2.439 | $0.274 | $3,555 |
| + Guardian v1 (0.90/2) | 13,078 | 67.06% | 3.211 | $0.286 | $3,746 |
| + HMM filter (TU_SHORT=0.80) | **12,582** | **67.32%** | **3.382** | **$0.296** | **$3,720** |

**Config final untuk holdout:**
- LGBM: thr_long=0.75, thr_short=0.70 (base) + TU_short=0.80
- Guardian v1: exit_thr=0.90, min_hold=2
- Holdout period: Apr 2026 – Jun 2026 (SEALED, belum dibuka)

---

### Eksperimen yang dicoba dan TIDAK dipakai

**LSTM Momentum Filter** (`ic32.rv2.lstm.11f.s72.c55`, `ic32_lstm_candidate_v2`):
- F1=0.3748 (nyaris random, baseline 0.333)
- Sweetspot LSTM_thr hanya ≥0.38 (masih noise)
- Net effect: PF naik +8% tapi volume -44% → PnL total lebih rendah dari LGBM alone
- Keputusan: SKIP

**LR Meta-learner** (`ic32_rv2_lr_meta_c55`):
- Stacking LGBM+LSTM OOF proba
- CV F1=0.5912, tapi threshold sweep → hasil identik LGBM alone
- Root cause: LR echo LGBM signal (lr_p2 = 0.87 pada LGBM LONG bars) — tidak bisa filter
- Keputusan: ABANDONED

**Guardian v2** (`ic32_rv2_guard_oof_v2`):
- 27 features, fix EXIT di negative PnL → 0%
- CV F1=0.8760 (sedikit lebih rendah dari v1)
- Trading result lebih buruk dari v1 di semua threshold
- Root cause: EXIT di neg PnL (v1 38.4%) justru berguna — Guardian belajar deteksi reversal
- Keputusan: GUNAKAN v1

**HMM filter agresif** (ban TREND_UP SHORT + ban RANGE_HV SHORT):
- PF naik ke 3.576 tapi trades turun ke 10,785 dan PnL -$412
- Tradeoff tidak menguntungkan
- Keputusan: hanya perketat TU_SHORT threshold (0.80), RANGE_HV tidak difilter

---

### Script (urutan eksekusi holdout)

```powershell
# Semua sudah selesai kecuali holdout:
python pipeline/07_holdout_ic32_regime_v2.py   # SEKALI — HANYA SETELAH INI DIBACA
```

**HOLDOUT_EVALUATED = False** — belum dijalankan.
...
## 2026-06-21 — LSTM Candidate Domain (ic32_lstm_candidate_v2)

**Status**: COMPLETED

### Hipotesis
LGBM dan LSTM memiliki edge fundamental yang berbeda:
- **LGBM** = snapshot analyst: membaca kondisi market saat ini (33 fitur tabular, interaksi cross-sectional)
- **LSTM** = historian analyst: membaca trajectory 72 bar menuju kondisi saat ini (path-dependency, temporal dynamics)

**Domain yang benar**: train LSTM pada bars di mana LGBM sudah cukup confident (p2 >= 0.55 OR p0 >= 0.55). LSTM lalu belajar: "given LGBM thinks this is directional, apakah trajectory 72 jam terakhir mendukung?"

### Yang Diubah
- **Domain filter**: `p2_lgbm >= CAND_THR OR p0_lgbm >= CAND_THR` (CAND_THR=0.55)
- **Coverage**: ~90% LGBM signal bars (vs 19% gate domain)
- **Class balance**: LONG=33.1%, FLAT=30.5%, SHORT=36.4% (near-balanced)
- RUN_NAME = "ic32_lstm_candidate_v2" | Script: `pipeline/05c_train_lstm_candidate_v2.py`
- SEQ_LEN=72, PURGE=72, 8 folds, 11 features, FocalLoss

### Hasil CV

| Fold | F1 | SHORT | FLAT | LONG |
|------|----|-------|------|------|
| 1 | 0.3745 | 0.355 | 0.302 | 0.466 |
| 2 | 0.3843 | 0.422 | 0.260 | 0.471 |
| 3 | 0.3681 | 0.439 | 0.239 | 0.427 |
| 4 | 0.3662 | 0.456 | 0.250 | 0.393 |
| 5 | 0.3808 | 0.488 | 0.210 | 0.444 |
| 6 | 0.3881 | 0.513 | 0.216 | 0.435 |
| 7 | 0.3744 | 0.498 | 0.172 | 0.453 |
| 8 | 0.3622 | 0.522 | 0.192 | 0.373 |
| **Mean** | **0.3748** | 0.462 | 0.230 | 0.433 |

**114,297 sequences** | 103,199 bars OOF | coverage ~90% LGBM signal bars

### Hasil Sweep OOF (Mode C >=0.33, LGBM threshold rendah)

| LGBM Thr | Trades/hari | WR% | PF | PnL | vs LGBM alone |
|----------|-------------|-----|----|-----|---------------|
| 0.72/0.67 | 4.9 | 63.1% | 2.423 | $2,800 | +0.7% WR, +0.18 PF |
| 0.73/0.68 | 4.5 | 63.9% | 2.520 | $2,664 | +0.9% WR, +0.21 PF |
| Baseline 0.75/0.75 alone | 3.7 | 67.4% | 2.807 | $2,627 | — |

### Kesimpulan
- F1=0.3748 — hanya sedikit di atas random (0.333 untuk balanced 3-class)
- Sweetspot hanya bisa pakai threshold LSTM>=0.33 (hampir noise) — tidak bertenaga cukup untuk >=0.40+
- FLAT F1 menurun fold demi fold (0.30→0.19) — LSTM belajar ignore FLAT (benar), tapi overall F1 flat
- SHORT F1 naik (0.36→0.52) — sisi positif: SHORT makin terprediksi seiring waktu
- **Root cause lemah**: 11 fitur average IC ~0.032 — terlalu lemah. Vol, OFI, log_ret saja tidak cukup konteks swing.
- Hipotesis tetap valid; perlu fitur lebih kuat (IC test menemukan mean IC bisa 2x lebih tinggi)

**Benchmark untuk v3**: F1=0.3748, sweetspot Mode C >=0.33 WR 63.9% PF 2.520 (LGBM 0.73/0.68)

### Script / Artefak
- `pipeline/05c_train_lstm_candidate_v2.py`
- `models/runs/ic32_lstm_candidate_v2/` — model, OOF, meta, scaler

---

## 2026-06-21 — LSTM Candidate v3: Fitur Lebih Kuat + SEQ_LEN=24

**Status**: PLANNED

### Hipotesis
ic32_lstm_candidate_v2 lemah bukan karena domain atau arsitektur, tapi karena **fitur IC rendah**. IC test pada 792k bars menunjukkan:
- Fitur terkuat yang TIDAK dipakai v2: `dist_from_8h_high` (IC 0.077), `ema_7_h1` (0.069), `rsi_h4` (0.058), `swing_momentum` (0.056), `ultra_high_vol` (0.201)
- Fitur terlemah yang DIPAKAI v2: `ofi_acceleration` (IC 0.005), `ofi_z_score` (0.019), `log_ret_20` (0.017)
- Rata-rata IC fitur lama ~0.032 vs baru ~0.055 (hampir 2x)

SEQ_LEN dari 72 → 24: swing H4 biasanya resolve dalam 24 bar (1 hari), bukan 3 hari. Sequence lebih pendek = lebih banyak training samples, lebih sedikit noise dari bars jauh.

### Yang Diubah
- **14 fitur baru** (drop 4 lemah dari v2, tambah 7 tinggi-IC baru):
  - Buang: `ofi_acceleration`, `ofi_z_score`, `log_ret_20`, `volume_delta`, `btc_h1_return`
  - Tambah: `dist_from_8h_high`, `ema_7_h1`, `rsi_h4`, `swing_momentum`, `stochrsi_k`, `trend_accel_4h`, `ultra_high_vol`, `dist_liq_50x_short`
- **SEQ_LEN = 24** (dari 72), PURGE = 24
- RUN_NAME = "ic32_lstm_candidate_v3"

### Target
- F1 macro >= 0.42 (vs benchmark v2: 0.3748)
- Sweetspot Mode C threshold bisa naik ke >=0.38–0.42 (vs v2 stuck di 0.33)
- WR >= 65%, PF >= 2.60 di config terbaik (vs v2: 63.9%, PF 2.520)
- Baseline: LGBM 0.75/0.75 alone (WR 67.4%, PF 2.807, $2,627)

### Script
- `pipeline/05d_train_lstm_candidate_v3.py`
