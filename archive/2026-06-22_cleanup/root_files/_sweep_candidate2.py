"""
Sweep LGBM threshold rendah x LSTM candidate — semua mode, full metrics.
"""
...
def _blank():
    return dict(t=0, w=0, pnl=0.0, sw=0.0, sl=0.0, lt=0, lw=0, st=0, sw2=0,
                veto=0, boost=0, skipped=0, no_data=0)
        ...
        for i in range(n):
            ...
            if mode == "A":
                ...
                if has[i]:
                    ...
                    if agree:
                        p_adj += AGREE_BOOST;  agg["boost"] += 1
                    elif opp:
                        p_adj -= OPP_PEN;      agg["veto"]  += 1
                else:
                    agg["no_data"] += 1
                if p_adj >= CONF_ENTRY:
                    y[i] = 2 if is_long else 0
...
def show(a, label, base=None):
    ...
    print(f"  {label:<36} {t:>7,}{dt:>8}  {wr:>5.1f}  {wrl:>5.1f}  {wrs:>5.1f}  "
          f"{pf:>6.3f}  {a['pnl']:>8.0f}{dp:>9}  {ppt:>7.4f}")


HDR = (f"  {'Config':<36} {'Trades':>15}  {'WR%':>5}  {'WRL%':>5}  {'WRS%':>5}  "
       f"{'PF':>6}  {'PnL':>17}  {'PPT':>7}")

CONFIGS = [
    (0.55, 0.50), (0.57, 0.52), (0.60, 0.55),
    (0.63, 0.58), (0.65, 0.60), (0.69, 0.59),
    (0.70, 0.65), (0.75, 0.70), (0.75, 0.75),
]

print("=" * 105)
print("  SWEEP LGBM threshold rendah x LSTM candidate_v2")
print("  WR | WR_LONG | WR_SHORT | PF | PnL | PPT")
print("=" * 105)

print(f"\n{HDR}")
print("--- MODE A: LGBM ONLY ---")
baselines = {}
...
print(f"\n{HDR}")
print("--- HARD CONSENSUS (veto p_opp > 0.50) ---")
for tl, ts in CONFIGS:
    a = run(tl, ts, "HC")
    show(a, f"HC  LGBM {tl}/{ts}", base=baselines[(tl, ts)])

print(f"\n{HDR}")
print("--- MODE C LSTM>=0.35 (strict agree or skip) ---")
for tl, ts in CONFIGS:
    a = run(tl, ts, "C", 0.35)
    show(a, f"C   LGBM {tl}/{ts} LSTM>=0.35", base=baselines[(tl, ts)])

print(f"\n{HDR}")
print("--- MODE C LSTM>=0.40 ---")
for tl, ts in CONFIGS:
    a = run(tl, ts, "C", 0.40)
    show(a, f"C   LGBM {tl}/{ts} LSTM>=0.40", base=baselines[(tl, ts)])

print(f"\n{HDR}")
print("--- MODE C LSTM>=0.45 ---")
for tl, ts in CONFIGS:
    a = run(tl, ts, "C", 0.45)
    show(a, f"C   LGBM {tl}/{ts} LSTM>=0.45", base=baselines[(tl, ts)])
