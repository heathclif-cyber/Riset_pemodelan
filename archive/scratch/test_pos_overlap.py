"""A/B test: Positioning Engine on overlap period (Jan-Oct 2025)"""
...
def add_p(df, coin):
    ...
    daily["oi_z20"] = (oi_t - om) / os; daily["oi_d1"] = oi_t.pct_change(1)
    if lsp is not None and "top_trader_position_ls" in lsp.columns:
        ls = lsp["top_trader_position_ls"]; daily["ls_pos_d7"] = ls.diff(7)
    ...
    if lsa is not None and "top_trader_account_ls" in lsa.columns:
        ...
        if "ls_pos" in daily.columns: daily["smart_retail"] = daily["ls_pos"] - daily["ls_acc"]
...
def run(coins, pos):
    config.POSITIONING_ENGINE_ENABLED = pos; btu.SMART_ENTRY_MODE = "disabled"
...
print("BASELINE..."); b = run(coins, False)
print("POSITIONING..."); p = run(coins, True)

print(f"\n{'='*60}")
print(f"  POSITIONING ENGINE A/B — Overlap Jan-Oct 2025, 5 coins")
print(f"  {'Metric':<20} {'BASELINE':>12} {'POS':>12} {'Delta':>10}")
print(f"  {'-'*55}")
for k, label in [("trades","Trades"),("wr","WR %"),("pnl","PnL $"),("pf","PF"),
                  ("lwr","LONG WR %"),("swr","SHORT WR %")]:
    bv = b[k]; pv = p[k]
    if isinstance(bv, float):
        print(f"  {label:<20} {bv:>12.1f} {pv:>12.1f} {pv-bv:>+10.1f}")
    else:
        print(f"  {label:<20} {bv:>12,} {pv:>12,} {pv-bv:>+10,}")
print(f"{'='*60}")
