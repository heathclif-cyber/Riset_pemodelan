"""Test Kelly sizing per HMM regime on extended backtest."""
...
KELLY_SIZE = {
    0: 0.18,  # TRENDING_DOWN - half-kelly
    1: 0.24,  # RANGING_LOW_VOL - half-kelly
    2: 0.30,  # RANGING_HIGH_VOL - half-kelly (highest edge)
    3: 0.19,  # TRENDING_UP - half-kelly
}
    ...
    for coin in TRAINING_COINS:
        ...
        for fi, (tr_idx, te_idx) in enumerate(folds):
            ...
            if mode == 'hmm_block':
                ...
                # Same entry logic as baseline, Kelly sizing applied post-trade
                pass  # Sizing applied after simulation

            elif mode == 'kelly_block':
                # Block counter-trend + Kelly sizing
                for i in range(n_te):
                    regime = int(df_te['hmm_regime_enc'].iloc[i])
                    h4_t = df_te['h4_trend'].iloc[i] if 'h4_trend' in df_te.columns else 0
                    if regime == 3 and yp[i] == 0 and h4_t > 0: yp[i] = 1
            ...
            for t in r.get('trades', []):
                ...
                if mode in ['kelly', 'kelly_block']:
                    regime = int(df_te['hmm_regime_enc'].iloc[t['bar_in']])
                    k = KELLY_SIZE.get(regime, 1.0)
                    t['net_pnl'] = t.get('net_pnl', 0) * k
...
for mode, label in [
    ('baseline', '1. BASELINE'),
    ('hmm_block', '2. HMM BLOCK counter-trend'),
    ('kelly', '3. KELLY half-sizing'),
    ('kelly_block', '4. KELLY + BLOCK'),
]:
