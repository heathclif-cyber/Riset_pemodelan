...
Jalankan: python pipeline/04_train_lgbm_genuine_v1.py
"""
...
logger = setup_logger("04_train_lgbm_genuine_v1")

RUN_NAME        = "tb_lgbm_genuine_v1"
FEAT_SOURCE_RUN = "tb_lgbm_flatboost_v2"   # feature list dari model yang sudah validated

# LGBM params — sama dengan flatboost_v2/v3 (tidak diubah agar perbandingan fair)
LGBM_PARAMS = {
...
# TB labeling params — sama dengan flatboost pipeline
...
def main():
    ...
    # Load feature list dari flatboost_v2 (validated 27 features)
    feat_json = MODEL_DIR / "runs" / FEAT_SOURCE_RUN / f"{FEAT_SOURCE_RUN}_features.json"
    ...
    sep = "=" * 70
    print(f"\n{sep}")
    print(f"  TB LGBM GENUINE OOF v1 — {RUN_NAME}")
    print(f"  Training period : 2020-01-01 -> {TRAIN_CUTOFF_DATE.date()}")
    print(f"  Features        : {len(FEATURES)} (from {FEAT_SOURCE_RUN})")
