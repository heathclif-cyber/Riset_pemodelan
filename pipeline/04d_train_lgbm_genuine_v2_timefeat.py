"""
pipeline/04d_train_lgbm_genuine_v2_timefeat.py -- LGBM Genuine OOF + time features

Ablation: genuine_v2 (34 feat) + hour_cos, hour_sin, dow_cos, dow_sin, market_session
Hipotesis: fitur waktu/jam bisa menangkap pola intraday yang tidak ada di baseline.

Kontrol sama dengan 04c_train_lgbm_genuine_v2.py. Holdout TIDAK dijalankan.
"""
...
logger = setup_logger("04d_train_lgbm_genuine_v2_timefeat")

RUN_NAME        = "tb_lgbm_genuine_v2_timefeat"
FEAT_SOURCE_RUN = "tb_lgbm_genuine_v1"
BASELINE_RUN    = "tb_lgbm_genuine_v2"
TIME_FEATS      = ["hour_cos", "hour_sin", "dow_cos", "dow_sin", "market_session"]
...
def main():
    ...
    with open(feat_json, encoding="utf-8") as f:
        rec = json.load(f)
    FEATURES = rec["recommended"] + TIME_FEATS

    sep = "=" * 70
    print(f"\n{sep}")
    print(f"  TB LGBM GENUINE OOF v2 + TIME FEATS -- {RUN_NAME}")
    print(f"  Base features    : 34 (genuine_v2)")
    print(f"  Added time feats : {TIME_FEATS}")
    print(f"  Total features   : {len(FEATURES)}")
    print(f"  Baseline compare : {BASELINE_RUN} (F1=0.3968, OOF PnL from cv_results.json)")
    print(f"  Holdout          : TIDAK DIJALANKAN")
    ...
    print(f"\nSTAGE 2: Purged Walk-Forward CV ({N_FOLDS} folds)...")
    print("-" * 55)
    ...
    print(f"\nSTAGE 3: Threshold Sweep via OOF...")
    print("-" * 55)
    ...
    imp  = sorted(zip(avail, gain), key=lambda x: -x[1])
    time_imp = {f: round(float(g), 1) for f, g in imp if f in TIME_FEATS}

    print(f"\n  Time feature gain importance:")
    for feat in TIME_FEATS:
        print(f"    {feat:<18} {time_imp.get(feat, 0):>10.1f}")

    baseline_cv = MODEL_DIR / "runs" / BASELINE_RUN / "cv_results.json"
    baseline_f1 = baseline_pnl = None
    if baseline_cv.exists():
        with open(baseline_cv, encoding="utf-8") as f:
            bl = json.load(f)
        baseline_f1  = bl.get("mean_f1_macro")
        baseline_pnl = bl.get("oof_pnl")
        print(f"\n  vs baseline {BASELINE_RUN}:")
        print(f"    F1  : {mean_f1:.4f} vs {baseline_f1} (delta {mean_f1 - baseline_f1:+.4f})")
        print(f"    PnL : ${best['pnl']:.2f} vs ${baseline_pnl} (delta ${best['pnl'] - baseline_pnl:+.2f})")
    ...
    with open(run_dir / "best_thresholds.json", "w") as f:
        json.dump({
            ...
            "pnl_per_trade": best["pnl_per_trade"], "sweep_method": "OOF_simulation",
            "sweep_all": sweep_results, "created": datetime.now().isoformat(),
        }, f, indent=2)
    ...
    cv_results = {
        ...
        "features": avail,
        "time_features_added": TIME_FEATS,
        "time_feature_importance_gain": time_imp,
        "baseline_run": BASELINE_RUN,
        "baseline_f1_macro": baseline_f1,
        "baseline_oof_pnl": baseline_pnl,
        "delta_f1_macro": round(mean_f1 - baseline_f1, 4) if baseline_f1 else None,
        "delta_oof_pnl": round(best["pnl"] - baseline_pnl, 2) if baseline_pnl else None,
        "n_samples": n_total,
        "n_folds": N_FOLDS,
        ...
        "folds": fold_metrics,
        "notes": "genuine_v2 + 5 time features. Holdout sealed.",
    }
