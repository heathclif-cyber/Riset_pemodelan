...
logger = setup_logger("06_train_guardian_genuine_v1")

LGBM_RUN_NAME = "tb_lgbm_genuine_v1"
RUN_NAME      = "tb_guardian_genuine_v1"
OUT_DIR       = MODEL_DIR / "runs" / RUN_NAME
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Static features — sama dengan profit_v1 (18 features yang sudah divalidasi)
STATIC_FEATS = [
...
def generate_oof_samples_for_coin(
    ...
    for t in trades:
        ...
        for j in range(bar_in + 1, min(bar_out, n)):
            ...
            # ── PROFIT-ONLY LABELING (sama dengan profit_v1) ──────────────────
            # TIDAK ADA loss-based EXIT — SL di evaluator yang handle itu
            label = None
