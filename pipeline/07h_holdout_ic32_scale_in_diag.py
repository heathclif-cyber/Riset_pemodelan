"""
Diagnostic holdout: scale_in vs multi-leg (sekali, konfirmasi OOF 08j).

Stack frozen: B-dir + hard_consensus + continuation_v1 + min_hold=4 + SL close.

Variants:
  no_pyr              — baseline 1 pos/koin
  pyr2_shared_sl_first — multi-leg live deployed
  pyr2_scale_in       — 1 trade, VWAP modal numpuk

Usage:
  python pipeline/07h_holdout_ic32_scale_in_diag.py
"""
import json
import sys
import warnings
...
def _scorecard(trades: list, variant: dict) -> dict:
    ...
    avg_modal = float(np.mean([t.get("modal_used", MODAL_PER_TRADE) for t in trades]))
    ppt = tpnl / n
    return {
        "label": variant["label"],
        "config": variant,
        ...
        "ppt_norm": round(ppt * (MODAL_PER_TRADE / avg_modal), 4) if avg_modal else 0,
        "avg_modal": round(avg_modal, 2),
        "avg_legs": round(float(np.mean([t.get("n_legs", 1) for t in trades])), 2),
        "pct_2leg": round(sum(1 for t in trades if t.get("n_legs", 1) >= 2) / n * 100, 1),
...
def _print_scorecard(sc: dict):
    ...
    print(f"  PPT             : ${sc['pnl_per_trade']:+.4f}  (norm ${sc.get('ppt_norm', 0):+.4f} @$10)")
    print(f"  Avg modal/legs  : ${sc.get('avg_modal', 10):.2f} / {sc.get('avg_legs', 1):.2f} ({sc.get('pct_2leg', 0):.1f}% 2-leg)")
...
def main():
    ...
    sep = "=" * 72
    print(f"\n{sep}")
    print("  HOLDOUT SCALE-IN DIAGNOSTIC — Apr-Jun 2026")
    print(f"{sep}")
    for v in VARIANTS:
