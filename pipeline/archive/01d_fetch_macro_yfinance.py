...
def fetch_macro_cross_asset() -> pd.DataFrame:
    ...
    for ticker, name in MACRO_TICKERS.items():
        try:
            ...
            # Normalize index to UTC midnight before concat
            ...
            out.index.name = "Date"
            out[col_close] = hist["Close"].values
            out[col_ret1] = pd.Series(hist["Close"].values, index=idx).pct_change(1)
            out[col_ret5] = pd.Series(hist["Close"].values, index=idx).pct_change(5)
            out[col_ret20] = pd.Series(hist["Close"].values, index=idx).pct_change(20)
