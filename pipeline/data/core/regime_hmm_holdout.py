...
SSOT holdout + live production (ic32_regime_v4). Live WAJIB mirror logika ini
(data_service._compute_hmm_regime). Lihat pipeline/data/CLAUDE.md.

Strategy:
  1. Fit HMM on training H4 data (< TRAIN_CUTOFF_DATE = 2026-04-01)
  2. Predict regime for ALL holdout H4 bars (>= cutoff) — NO tail(N)
  3. Forward-fill to H1, save to HOLDOUT_DIR/labeled/{coin}_regime_h1.parquet
  4. Export models/hmm/{coin}_hmm.pkl untuk inferensi live

OOS-clean: model tidak lihat holdout data saat fitting.
Bukan regime_hmm.py (walk-forward OOF + BTC) — itu hanya label training.
"""
...
from core.regime import fit_hmm, predict_hmm_causal, encode_regime
...
def process_coin(coin: str) -> bool:
    try:
        ...
        # Predict regime for holdout -- causal filtering (bukan Viterbi): state bar t
        # cuma dari observasi sampai t, tidak pernah "intip" bar sesudahnya. Viterbi lama
        # decode GLOBAL atas seluruh window holdout yang terus bertambah panjang tiap
        # re-run -- itu lookahead bias di reproduksi riset (state bar lama bisa berubah
        # retroaktif), BUKAN cara live sungguhan mengambil keputusan. Ganti 2026-07-13
        # setelah audit compare_oos_live_signals.py + re-tuning threshold (base 0.65->0.70,
        # delta tetap 0.10). Lihat EXPERIMENTS.md 2026-07-13.
        h4_labels = predict_hmm_causal(model, df_holdout, state_map)
