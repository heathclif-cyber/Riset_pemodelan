...
flowchart LR
    ...
    C -->|03_engineer.py| D[Features V3 Parquet<br/>83 fitur + label]
...
│   ├── migrate_registry.py      ← Konversi model_registry.json lama → format versioned (jalankan sekali)
│   ├── prepare_deploy.py        ← Buat folder v{run_id}/, copy model files, copy inference_config.json
...
CREATE TABLE model_meta (
    ...
    n_features            INTEGER DEFAULT 83,      -- lock jumlah fitur saat training
...
| [`core/features.py`](core/features.py) | `engineer_features()`, `calc_atr()`, `calc_rsi()` | Hitung 83 fitur |
...
|--------|---------------|--------|
| `prepare_latest_features(symbol, n_bars=250)` | Fetch klines 1h/4h/1d + macro → clean + MTF join → `engineer_features()` → set `long_short_ratio=0` | `DataFrame` (83 fitur + h4_swing_high/low) atau `None` jika data tidak cukup |
| `prepare_lstm_input(df, scaler)` | Scale `df[FEATURE_COLS_V3]` → sequence `LSTM_SEQ_LEN=32` terakhir → reshape | `ndarray (1, 32, 83)` |
...
|------------|-------------------|
| **lstm** (Fase 1) | `features_df` → scale → sequence `(1, 32, 83)` → LSTM forward → softmax → proba |
...
```json
{
  "versions": [
    {
      "run_id": "20250430_143000",
      "model_type": "lstm",
      "n_features": 83,
      "trained_at": "2025-04-30T14:30:00Z",
      "evaluated_at": "2025-04-30T16:00:00Z",
      "backtest_summary": {
        "mean_winrate": 0.63,
        "mean_drawdown_lev3x": 0.18
      },
      "coins_validated": ["SOLUSDT", "ETHUSDT", "BNBUSDT"],
      "paths": {
        "lstm": "models/v20250430_143000/lstm_best.pt",
        "scaler": "models/v20250430_143000/lstm_scaler.pkl",
        "inference_config": "models/v20250430_143000/inference_config.json"
      },
      "status": "available"   // available | active | deprecated
    }
  ]
}
```
...
`deploy/prepare_deploy.py:export_lstm_to_onnx()` — load `TradingLSTM` → `torch.onnx.export()` dengan `dummy_input = torch.randn(1, 32, 83)`, `opset_version=17`, dynamic batch axis.
...
| Feature DataFrame (1 coin, **250 bars**, 83 cols) | ~2 MB | ~8 MB | |
...
| `feature_cache` | 5 | 5 menit | Feature DataFrame per coin (250 bars × 83 cols) |
...
flowchart TD
    ...
    C --> D[4. Verifikasi lokal<br/>- inference_config.json terbentuk<br/>- n_features == 83<br/>- model_registry.json format baru]
...
- [ ] Training pipeline selesai: `04` → `05` → `06` → `07` → `08` → `09`
- [ ] Buat `deploy/migrate_registry.py` — konversi `model_registry.json` ke format versioned sebelum training berikutnya
- [ ] Tambahkan 1 baris di `08_backtest.py`: salin `inference_config.json` ke `models/v{run_id}/inference_config.json`
- [ ] Verifikasi `models/v{run_id}/inference_config.json` berisi `n_features: 83`
